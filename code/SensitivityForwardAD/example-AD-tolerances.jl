...
grad0 = Zygote.gradient(
    p -> sum(solve(prob, Tsit5(), u0 = u0, p = p, sensealg = ForwardSensitivity(),
        saveat = 0.1, abstol = abstol, reltol = reltol)),
    p)
# grad0 = ([212.71042521681443],)
@show grad0

# Original AD with wrong norm
grad1 = Zygote.gradient(
    p -> sum(solve(prob, Tsit5(), u0 = u0, p = p, sensealg = ForwardDiffSensitivity(),
        saveat = 0.1, internalnorm = (u, t) -> sum(abs2, u / length(u)),
...
sol1 = solve(prob, Tsit5(), u0 = u0, p = p, sensealg = ForwardDiffSensitivity(),
    internalnorm = (u, t) -> sum(abs2, u / length(u)), abstol = abstol, reltol = reltol)
times1 = sol1.t
...
grad3 = Zygote.gradient(
    p -> sum(solve(
        prob, Tsit5(), u0 = u0, p = p, sensealg = ForwardDiffSensitivity(), saveat = 0.1,
