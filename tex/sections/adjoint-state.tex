...
To gain further intuition about the adjoint method, let us consider the simple case of the explicit linear one-step methods, where at every step we need to solve the equation $g_i (u_{i+1}; \theta) = u_{i+1} - A_i (\theta) \, u_i - b_i(\theta)$, where $A_i(\theta) \in \R^{n \times n}$ and $b_i(\theta) \in \R^n$ are defined by the numerical solver. 
