...
Some common numerical solver consists in linear multistep methods of the form 
\begin{equation}
...
Explicit methods are characterized by $\beta_{n, 0} = 0$, otherwise, the method is implicit. 

For simplicity, let us consider an explicit numerical solver of the form 
\begin{equation}
...
If we call the super-vector $U = (u_1, u_2, \ldots, u_N) \in \R^{nN}$, we can combine all these equations into one single system of linear equations 
\begin{equation}
    A(\theta) U 
    = 
    \begin{bmatrix}
