...
Examples of these data structures include dual numbers and Wengert lists \cite{Baydin_Pearlmutter_Radul_Siskind_2015}.
...
We define a dual number based on two variables: a \textit{value} coordinate $x_1$ that carries the value of the variable and a \textit{derivative} (o partial) coordinate $x_2$ with the value of the derivative $\frac{\partial x_1}{\partial \theta}$. 
...
we need to know the state value of all the argument variables $v$ of $\omega$ in order to evaluate the terms $\frac{\partial w}{\partial v}$.
...
Full Jacobians like the sensitivity $s = \frac{\partial u}{\partial \theta} \in \R^{n \times p}$ can be fully reconstructed by the independent computation of the $p$ columns of $s$ via the JVPs $s e_i$, with $e_i \in \R^p$ the canonical vectors; or by the calculation of the $m$ rows of $s$ via the VJPs $e_j^T s$, with $e_j \in \R^n$.
...
Intepreatation without multiarray Jacobian computation.
