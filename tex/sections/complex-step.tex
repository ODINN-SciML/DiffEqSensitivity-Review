An alternative to finite differences that avoids substractive cancellation errors is based on complex variable analysis. 
...
However, a major limitation of this method is that it works just for complex analytical functions \cite{Martins_Sturdza_Alonso_2003_complex_differentiation} and does not outperforms AD. 
One additional limitation is that it requires the evaluation of mathematical functions with small complex values, e.g. operations like $\sin(1 + 10^{-16} i)$, which are not necessarily always computable to high accuracy with modern math libraries.
