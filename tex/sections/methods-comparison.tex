...
\begin{equation}
    \bar g_m
    = 
    \frac{\partial u^{m+1}}{\partial g_m} \bar u^{m+1}
    = 
    \frac{\partial L}{\partial u^{m+1}} 
    +
    \frac{\partial g_{m+1}}{\partial u^m} \bar g_{m+1}.
\end{equation}
The updates of $\bar g_m$ then mathematically coincide with the updates in reverse mode of the adjoint variable $\lambda_m$ (see Equation \eqref{eq:adjoint-discrete-linear-example}).
...
This is certainly the case for implicit methods when $u^{m+1}$ is the solution of an iterative Newton method \cite{SUNDIALS-hindmarsh2005sundials}; or in cases where the numerical solver includes internal iterative sub-routines \cite{Alexe_Sandu_2009}.
...
\begin{split}
    u^{m+1} + s^{m+1} \, \epsilon 
    &= 
    u^m +  s^m \, \epsilon + \Delta t \, f (u^m + s^m \, \epsilon, \theta + \epsilon, t_m) \\
    &= 
