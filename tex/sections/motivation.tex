...
Mechanistic (or process-based) models have played a central role in a wide range of scientific disciplines. They consist of precise mathematical descriptions of causal mechanisms, assumed to be the main processes underlying the system behavior \cite{rackauckas2020universal}. 
Differential equations are particularly convenient mathematical representations that, together with the numerical methods to approximate their solutions, led to fundamental advances in the understanding and prediction of physical and biological systems since their invention.
...
Crucially, differential equation models depend on parameters that determine the processes represented. The parameter values have traditionally been estimated independently of the model, with empirical data for model validation and comparison \cite{hartig2012}.
The independent estimation of parameters poses several problems.
First, the independent estimation of parameters and processes rapidly becomes impossible as the number of variables modelled increases, especially when considering highly non-linear processes. 
Second, 
...
\begin{itemize}
    ...
    \item \textbf{Model selection} Estimating the model evidence for each candidate model to discriminate between competing hypotheses \cite{burnham2004}. Involves the calculation of the marginal likelihood for each model, which may be approximated by Laplace approximation and involves the calculation of the hessian of the likelihood function, or by the calculation of information-based criteria (AIC, BIC, see ref\cite{burnham2002}).
\end{itemize}
...
hu2010supersonic, fike2013multi}, biology \citep{Strouwen2022}
...
Differential equation models have been broadly used in biology and ecology to model the functioning and function of cells \cite{Gabor2015}, eco-evolutionary dynamics \cite{Lion2018,Villa2021,Boussange2022,boussange2023a}, population dynamics and animal movement \cite{Volpert2009}, the dynamics of microbial \cite{VandenBerg2022} or ecological communities \cite{Akesson2021,chalmandrier2021}, and biomass and energy fluxes and transformation at ecosystem levels \cite{Weng2015,Schartau2017,Franklin2020,Geary2020}.
Conducting microcosm experiments to estimate biological parameters in biological models is costly.
Furthermore, parameters independently inferred from microsocms experiments usually fail in capturing real biological dynamics \cite{Watts2001}. 
As a consequence, inverse modelling methods have great potential in biology and ecology.
...
In model selection, candidate models embedding competing hypotheses about causal processes are derived, and the relative support of each model given the data is computed to discriminate between the hypotheses \cite{Johnson2004}.
...
While inverse modelling in biology has been mostly agnostic to AD techniques, they potential have recently been highlighted \cite{frank2022}. 
\cite{alsos2023} suggests to take advantage of increasingly available data such as paleao-time series with inverse modelling techniques to recover the biotic and abiotic processes underlying ecosystem dynamics. 
Dynamic models informed by data can further be used to extrapolate beyond current dynamics and provide robust forecasts of ecosystem responses to future climate change \cite{alsos2023}. 
Physics-informed approaches have also emerged recently \cite{Yazdani2020}.