...
\end{equation}
where $u \in \mathbb{R}^n$ is the unknown solution; $f: \R^n \times \R^p \times \R \mapsto \R^n$ is a function that depends on the state $u$, some vector parameter $\theta \in \mathbb R^p$, and potentially the independent variable $t$ which we will refer to as time; and with initial condition $u(t_0) = u_0$.
...
Numerical solvers for the solution of ODEs or initial value problems (IVP) can be classified as one-step methods, among which Runge-Kutta methods are the most widely used, and multi-step methods \cite{hairer-solving-1}.
...
% Stiffness
There are many considerations when picking a numerical solver. 
One of the most important ones is the stiffness of the differential equation we are trying to solve. 
Although stiffness is a known phenomenon in the study of differential equation solver, different definitions and types of instability exist in the literature. 
...
\end{itemize} 
Stability properties can be achieved by different means, for example by the use of implicit methods or stable explicit methods, such as Runge–Kutta–Chebyshev. 
...
\begin{itemize}
    ...
    More generally, misfit functions used in optimal estimation and control problems are composite maps from the parameter space $\theta$ via model's state space, in this case the solution $u(t)$, to the observation space defined by a new variable $y(t) = H(u(t, \theta))$, where $H: \R^n \mapsto \R^o$ is a given function mapping the latent state to observational space \cite{1975-Bryson-Ho-optimal-control}. 
    ...
    \end{equation}
    Provided with a prior distribution $p(\theta)$ for the parameter $\theta$, we can further compute a posterior distribution for $\theta$ given the observations $Y$ following Bayes theorem as $p(\theta | Y) = {p(Y | \theta) \, p (\theta)}/{p(Y)}$ and further estimate a value for $\theta$ as the posterior mean of the maximum a posteriori (MAP) \cite{pml1Book}.
    % \begin{equation}
    ...
    \item \textbf{Quantity of interest.} Another important example is when $L$ returns the value of the solution at one or many points, which is useful when we want to know how the solution itself changes as we move the parameter values; or more generally when it returns the value of some variable that is a function of the solution of a differential equation. The later corresponds to the case in design control theory, a popular approach in aerodynamics modelling where goals include maximizing the speed of an airplane or the lift of a wing given the solution of the flow equation for a given geometry profile \cite{Jameson_1988,Giles:2000wp,Mohammadi:2004dg}. 
    % \todo[inline]{PH: I do not see the difference between a QoI and a "Diagnosis of the solution". Both refer to general scalar-valued functions whose sensitivity we seek (any QoI can be any Diagnosis). I would merge these.}
\end{itemize}
...
While a direct implementation of gradient descent is prone to converge to a local minimum and slows down in a neighborhood of saddle points, variants employing more advanced updating strategies have been proposed to avoid convergence to local minima \cite{ruder2016overview-gradient-descent}.
...
% Just as in this last example, 
% \todo{\footnotesize PH: Probably not true, since most of these applications also have regularization/penalty terms, such that $L$ depends directly on the parameters.}
% in most applications the loss function $L(\theta)$ will depend on $\theta$ just through $u$, meaning $\frac{\partial L}{\partial \theta} = 0$.
...
\end{equation}
The sensitivity $s$ defined in Equation \eqref{eq:sensitivity-definition} is a \textit{Jacobian}, that is, a matrix of first derivatives for general vector-valued functions.
