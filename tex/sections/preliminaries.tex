...
\end{equation}
where $u \in \mathbb{R}^n$ is the unknown solution; $f$ is a function that depends on the state $u$, some parameter $\theta \in \mathbb R^p$, and potentially time $t$; and with initial condition $u(t_0) = u_0$.
...
\begin{itemize}
    \item \textbf{Loss functions}. This is usually a real valued function that quantifies the accuracy or prediction power of a given model. Examples of loss functions include the squared error
    \begin{equation}
    ...
    \item \textbf{Likelihood profiles. }   
