...
\begin{equation}
    ...
    0.
\end{equation}
with coefficients $\alpha_{ni}$ and $\beta_{nj}$
...
\begin{itemize}
    \item Stiff differential equations are characterized by dynamics with different time scale \cite{kim_stiff_2021}, also characterized by the phenomena of increasing oscillations...
...
% Generalization 
It is important to remark that direct methods to solve higher order systems of ODEs exist (Nystrom), but we can usually reduce them to a extended systems of ODEs \cite{hairer-solving-1}. 
% PDE, meshes, ... 
