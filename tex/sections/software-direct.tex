...
\begin{align}
    ...
    &\sum_{i=1}^n \left( \frac{u_i^{m} - \hat u_i^{m}}{\mathfrak{abstol} + \mathfrak{reltol} \cdot \max \{ u_i^{m}, \hat u_i^{m}\}} \right)^2 \nonumber \\
    + 
    &\sum_{i=1}^n \sum_{j=1}^p  
    \left( \frac{s_{ij}^{m} - \hat s_{ij}^{m}}{\mathfrak{abstol} + \mathfrak{reltol} \cdot \max \{ s_{ij}^{m}, \hat s_{ij}^{m}\}} \Bigg)^2 \right)
    \Bigg ]^{\frac{1}{2}},
