...
\begin{enumerate}[label=(\roman*)]
    ...
    However, this requires cubic time in the total number of ordinary differential equations, leading to a total complexity of $\mathcal O((n+p)^3)$ for the adjoint method. In practice, this methods is hardly stable for most complex (even non-stiff) differential equations. 
\end{enumerate} 
