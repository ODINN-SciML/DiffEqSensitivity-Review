...
\begin{enumerate}[label=(\roman*)]
    \item How to handle VJPs and/or JPVs 
    \item Stability of the numerical solver, including the original ODE but also the sensitivity/adjoint equations
...
For the adjoint equations, we need to evaluate the term $\lambda^T \frac{\partial G}{\partial \theta}$ for the discrete adjoint method in Equation \eqref{eq:def_adjoint}, and $\lambda^T \frac{\partial f}{\partial \theta}$ for the continuous adjoint method in Equation \eqref{eq:casa-final-loss-gradient}. 
...
These will compute the JVPs/VJPs using some form of AD, which will result in correct calculations with different performance trade-offs. 
...
\begin{jllisting}
    complete
\end{jllisting}
Notice that here we used the final state of the solution $u$ of the differential equation as starting point, which is then recalculated in backwards direction. 
