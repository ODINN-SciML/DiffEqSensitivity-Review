...
In this section, we are going to address how these tools are computationally implemented and how to decide which method to use depending on the problem. 
...
The total number of equations and parameters is the main limiting factor depending on the available computational resources. 
...
\end{enumerate}
These considerations also vary depending on the sensitivity method directly depends on the differential equation and the numerical solver used to solve it or if instead, the gradient is computed independently of the internals of the solver. 
...
In general, finite differences are very easy to implement since it does not require any type of software support, but it is less accurate and as costly as forward AD \cite{Griewack-on-AD} and complex-step differentiation. 
Implementing these last types in a programming language implies defining what it means to perform basic operations and then combining them using the chain rule provided by the dual number properties.
In Julia, we can create a dual number by defining an object with two values, and then extend the definition of algebraic operations and functions, a process known as \textit{operator overloading} \cite{Neuenhofen_2018}, by relying on multiple dispatch:
\begin{jllisting}
using Base: @kwdef
...
Both methods introduce an abstract unit ($\epsilon$ and $i$, respectively) associated with the imaginary part of the extender value that carries forward the numerical value of the gradient. 
...
However, although these methods seem very similar, it is important to remark that AD gives the exact gradient, while complex step differentiation relies on numerical approximations that are valid just when the stepsize $\varepsilon$ is small. 
...
On the other hand, it is important to remark that the underlying machinery of all solvers relies on solvers for linear systems of equations, which can be solved in dense, band (sparse), and Krylow mode. 
